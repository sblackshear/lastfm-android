...
public class MP3Streamer
{
    ...
    public MP3Streamer(Context context, URL streamUrl, int bitrate)
    {
    ...
    {
        final CompletionListener listener = mCompletionListener;
        if(listener != null) {
            // Run the listener on its own thread.
            mHandler.post(new Runnable() {
                public void run() {
                    listener.onMP3Completion();
                }
            });
        }
